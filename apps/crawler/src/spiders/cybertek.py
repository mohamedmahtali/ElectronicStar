"""Spider Cybertek — PC, composants & périphériques gaming.

Cybertek ne nécessite pas Playwright : pages ASP.NET statiques.
JSON-LD Product complet sur chaque fiche produit.

SKU : identifiant numérique en fin d'URL, ex. -160744.aspx → '160744'.
"""
import json
import re
...
# SKU en fin d'URL : ex. -160744.aspx ou -154241.aspx
_SKU_RE = re.compile(r"-(\d{5,})\.aspx", re.I)


class CybertekSpider(BaseProductSpider):
    name = "cybertek"
    merchant_slug = "cybertek"
    allowed_domains = ["www.cybertek.fr"]

    start_urls = [
        "https://www.cybertek.fr/pc-portable-40.aspx",
        "https://www.cybertek.fr/macbook-120.aspx",
        "https://www.cybertek.fr/tv-67.aspx",
        "https://www.cybertek.fr/micro-casque-73.aspx",
        "https://www.cybertek.fr/ecran-pc-62.aspx",
    ]
    ...
    def errback_log(self, failure):
        self.logger.error("Erreur Cybertek %s: %r", failure.request.url, failure.value)
    ...
    def _from_css(self, response) -> RawItem | None:
        sku = _sku_from_url(response.url)
        if not sku:
            self.logger.warning("SKU Cybertek introuvable : %s", response.url)
            return None
